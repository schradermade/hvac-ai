...
- `services/diagnosticService.ts` - Business logic (API calls, data transformation)
- `hooks/useDiagnostic.ts` - React hooks for state management
...
function MyComponent() {
  const { data, isLoading } = useDiagnostic('123');
...
```bash
npm test -- src/features/diagnostic
```
